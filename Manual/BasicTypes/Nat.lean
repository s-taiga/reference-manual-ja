...
{deftech}[自然数] （natural number）は非負の整数です。論理的には 0・1・2・3・……であり、コンストラクタ {lean}`Nat.zero` と {lean}`Nat.succ` から生成されます。Lean はコンピュータの利用可能なメモリによって課される物理的制約以外に、自然数の表現に上限を課していません。
...
自然数リテラルは {lean}`OfNat` 型クラスを使って上書きされます。これは {ref "nat-literals"}[リテラルの構文の節] で記述されています。
