...
単一の演算子についてその配置と字句を示す代わりに、記法の本体の宣言は {deftech}_記法アイテム_ （notation item）の列から構成されます。これは新しい {tech}[atoms] （`if` や `#eval` などのキーワードと `=>` ・ `+` ・ `↗` ・ `⟦` ・ `⋉` などの記号の両方を含む）、または項の位置です。演算子と同様に、文字列リテラルはアトムの配置を指定します。文字列の先頭と末尾の空白はパースに影響しませんが、空白を入れた場合 {tech}[証明状態] やエラーメッセージで構文を表示する時に Lean が対応する位置に空白が挿入されます。識別子は項が予想される位置を示し、対応する項を記法の展開に挿入できるように名前を付けます。
