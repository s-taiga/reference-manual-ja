 ...
 * {lean}`Nat` は `lean_object *` で表現されます。そのランタイム値は不透明な任意精度整数オブジェクトへのポインタか、または「ポインタ」の最下位ビットが `1` の場合（`lean_is_scalar` でチェックされます）、ボックス化解除された自然数にエンコードされます（`lean_box`/`lean_unbox`）。
